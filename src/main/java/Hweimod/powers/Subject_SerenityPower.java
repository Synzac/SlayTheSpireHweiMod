...
public class Subject_SerenityPower extends AbstractPower {
    public static final String POWER_ID = ModHelper.makePath(Subject_SerenityPower.class.getSimpleName());
    ...
    private static final String PATH128 = ModHelper.makePowerAd(Subject_SerenityPower.class.getSimpleName(), true);
    private static final String PATH48 = ModHelper.makePowerAd(Subject_SerenityPower.class.getSimpleName(), false);

    public Subject_SerenityPower(AbstractCreature owner) {
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
            ...
            if (card.target == AbstractCard.CardTarget.ALL_ENEMY) {
                for (AbstractMonster mo : (AbstractDungeon.getCurrRoom()).monsters.monsters) {
                    if (!mo.hasPower(Color_SerenityPower.POWER_ID) && !mo.hasPower(Color_DisasterPower.POWER_ID)
                        ...
                        addToBot(new ApplyPowerAction(mo, this.owner, new Color_SerenityPower(mo, 1), 1, true, AbstractGameAction.AttackEffect.NONE));
                ...
                if (!(action.target == null) &&
                    ...
                    addToBot(new ApplyPowerAction(action.target, this.owner, new Color_SerenityPower(action.target, 1), 1, true, AbstractGameAction.AttackEffect.NONE));
                ...
                if (!this.owner.hasPower(Color_SerenityPower.POWER_ID) && !this.owner.hasPower(Color_DisasterPower.POWER_ID)
                    ...
                    addToBot(new ApplyPowerAction(this.owner, this.owner, new Color_SerenityPower(this.owner, 1), 1, true, AbstractGameAction.AttackEffect.NONE));
    ...
    public void onSpecificTrigger() {
        addToTop(new RemoveSpecificPowerAction(this.owner, this.owner, Subject_SerenityPower.POWER_ID));
    }
