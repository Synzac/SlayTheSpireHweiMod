...
public class Subject_DisasterPower extends AbstractPower {
    public static final String POWER_ID = ModHelper.makePath(Subject_DisasterPower.class.getSimpleName());
    private static final PowerStrings powerStrings = CardCrawlGame.languagePack.getPowerStrings(POWER_ID);
    private static final String NAME = powerStrings.NAME;
    private static final String[] DESCRIPTIONS = powerStrings.DESCRIPTIONS;
    private static final String PATH128 = ModHelper.makePowerAd(Subject_DisasterPower.class.getSimpleName(), true);
    private static final String PATH48 = ModHelper.makePowerAd(Subject_DisasterPower.class.getSimpleName(), false);

    public Subject_DisasterPower(AbstractCreature owner) {
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
    ...
    public void onInitialApplication() {
        ...
        if(this.owner.hasPower(Subject_DespairPower.POWER_ID)){
            this.owner.getPower(Subject_DespairPower.POWER_ID).onSpecificTrigger();
        }
        ...
        for (AbstractCard card : AbstractDungeon.player.discardPile.group){
            if (Objects.equals(card.cardID, XuRi.ID)){
            ...
            if (card.target == AbstractCard.CardTarget.ALL_ENEMY) {
                for (AbstractMonster mo : (AbstractDungeon.getCurrRoom()).monsters.monsters) {
                    if (!mo.hasPower(Color_SerenityPower.POWER_ID) && !mo.hasPower(Color_DisasterPower.POWER_ID)
                        ...
                        addToBot(new ApplyPowerAction(mo, this.owner, new Color_DisasterPower(mo, 1), 1, true, AbstractGameAction.AttackEffect.NONE));
                ...
                if (!(action.target == null) &&
                    ...
                    addToBot(new ApplyPowerAction(action.target, this.owner, new Color_DisasterPower(action.target, 1), 1, true, AbstractGameAction.AttackEffect.NONE));
                ...
                if (!this.owner.hasPower(Color_SerenityPower.POWER_ID) && !this.owner.hasPower(Color_DisasterPower.POWER_ID)
                    ...
                    addToBot(new ApplyPowerAction(this.owner, this.owner, new Color_DisasterPower(this.owner, 1), 1, true, AbstractGameAction.AttackEffect.NONE));
    ...
    public void onSpecificTrigger() {
        addToTop(new RemoveSpecificPowerAction(this.owner, this.owner, Subject_DisasterPower.POWER_ID));
    }
