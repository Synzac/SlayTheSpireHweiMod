...
public class Subject_TormentPower extends AbstractPower {
    public static final String POWER_ID = ModHelper.makePath(Subject_TormentPower.class.getSimpleName());
    ...
    private static final String PATH128 = ModHelper.makePowerAd(Subject_TormentPower.class.getSimpleName(), true);
    private static final String PATH48 = ModHelper.makePowerAd(Subject_TormentPower.class.getSimpleName(), false);

    public Subject_TormentPower(AbstractCreature owner) {
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
            ...
            if (card.target == AbstractCard.CardTarget.ALL_ENEMY) {
                for (AbstractMonster mo : (AbstractDungeon.getCurrRoom()).monsters.monsters) {
                    if (!mo.hasPower(Color_SerenityPower.POWER_ID) && !mo.hasPower(Color_DisasterPower.POWER_ID)
                        ...
                        addToBot(new ApplyPowerAction(mo, this.owner, new Color_TormentPower(mo, 1), 1, true, AbstractGameAction.AttackEffect.NONE));
                ...
                if (!(action.target == null) &&
                    ...
                    addToBot(new ApplyPowerAction(action.target, this.owner, new Color_TormentPower(action.target, 1), 1, true, AbstractGameAction.AttackEffect.NONE));
                ...
                if (!this.owner.hasPower(Color_SerenityPower.POWER_ID) && !this.owner.hasPower(Color_DisasterPower.POWER_ID)
                    ...
                    addToBot(new ApplyPowerAction(this.owner, this.owner, new Color_TormentPower(this.owner, 1), 1, true, AbstractGameAction.AttackEffect.NONE));
    ...
    public void onSpecificTrigger() {
        addToTop(new RemoveSpecificPowerAction(this.owner, this.owner, Subject_TormentPower.POWER_ID));
    }
